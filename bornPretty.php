...
<head>
    ...
    <title>born pretty Products | Gimmick</title>
...
<body>
    ...
    <div class="row">
     <?php 
       ...
       if(isset($_GET["page"])){
        ...
        $product=Database::search("SELECT * FROM `product` INNER JOIN `brand_has_model` ON `product`.`brand_has_model_id`=`brand_has_model`.`id`
        INNER JOIN `brand` ON `brand_has_model`.`brand_id`=`brand`.`id` WHERE `brand`.`name`='Born Pretty'");
        ...
        $selected_rs=Database::search("SELECT `product`.`id` FROM `product` INNER JOIN `brand_has_model` ON `product`.`brand_has_model_id`=`brand_has_model`.`id`
        INNER JOIN `brand` ON `brand_has_model`.`brand_id`=`brand`.`id` WHERE `brand`.`name`='Born Pretty' LIMIT " . $results_per_page . " OFFSET " . $page_results . "");
    ...
    <?php
     if($selected_num==0){
        ?>
         <h2 style="margin-top:17vh; margin-left:30%">No items. Let's start shopping from another!</h2>
        <a href="allProducts.php"class="btn btn-primary fs-3 mb-5" style="height:7vh ; border-radius:15px; margin-left:35%; width:30%;">Start Shopping</a>
