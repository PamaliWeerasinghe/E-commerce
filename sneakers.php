    ...
    <div class="row">
     <?php 
       ...
       if(isset($_GET["page"])){
        ...
        $product=Database::search("SELECT `product`.`id` FROM `product` INNER JOIN `category` ON `category`.`id`=`product`.`category_id` WHERE `category`.`name`='Sneakers'");
        ...
        $selected_rs=Database::search("SELECT `product`.`id` FROM `product` INNER JOIN `category` ON `category`.`id`=`product`.`category_id` WHERE `category`.`name`='Sneakers' LIMIT " . $results_per_page . " OFFSET " . $page_results . "");
