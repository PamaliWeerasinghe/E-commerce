    ...
    <div class="row">
     <?php 
       ...
       if(isset($_GET["page"])){
        ...
        $product=Database::search("SELECT `product`.`id` FROM `product` INNER JOIN `category` ON `category`.`id`=`product`.`category_id` WHERE `category`.`name`='Mobiles'");
        ...
        $selected_rs=Database::search("SELECT `product`.`id` FROM `product` INNER JOIN `category` ON `category`.`id`=`product`.`category_id` WHERE `category`.`name`='Mobiles' LIMIT " . $results_per_page . " OFFSET " . $page_results . "");
