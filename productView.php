...
<body style="background-color:white ;">
    ...
    $pro=Database::search("SELECT * FROM `product` WHERE `id`='".$pid."'");
    ...
    $count=Database::search("SELECT * FROM `search_product` WHERE `user_email`='".$email."' AND `product_id`='".$pid."'");
    $count_data=$count->num_rows;
    if($count_data==0){
        Database::iud("INSERT INTO `search_product`(`product_id`,`user_email`,`qty`) VALUES('".$pid."','".$email."','1')");
    ...
    $condition=Database::search("SELECT * FROM `condition` INNER JOIN `product` ON `product`.`condition_id`= `condition`.`id` WHERE `product`.`id`='".$pid."' ");
    $condition_data=$condition->fetch_assoc();
    $colour=Database::search("SELECT * FROM `colour` INNER JOIN `product` ON `product`.`colour_id`=`colour`.`id` WHERE `product`.`id`='".$pid."' ");
    $colour_data=$colour->fetch_assoc();
    $category=Database::search("SELECT * FROM `category` INNER JOIN `product` ON `product`.`category_id`=`category`.`id` WHERE `product`.`id`='".$pid."'  ");
    ...
    <div class="container-fluid">
        ...
        <div class="row mt-1">
            <div class="col-lg-1">
                <div class="row">
                    ...
                    for ($x=0; $x<$images_num;$x++){
                        ...
                        ?>
                        <img src="<?php echo $images_data["path"] ?>" class="image2 border" id="productImg<?php echo $x; ?>" onclick="loadMainImg(<?php echo $x; ?>);" onload="mainImg(<?php echo $x; ?>)"/>
                        <?php
                    }
            ...
            <?php 
            $images1=Database::search("SELECT * FROM `images` WHERE `product_id`='".$pid."'");
            $images1_data=$images1->fetch_assoc();
            $img1=$images1_data["path"];
            ?>
            <img src="<?php echo $img1?>" id="mainimg1" class="thumbnail" style="" />
            <?php
            ?>
            </div>
            <div class="col-lg-7">
                <div class="row">
                    <?php 
                    $amount=Database::search("SELECT * FROM `product` WHERE `id`='".$pid."'");
                    $amount_data=$amount->fetch_assoc();
                    $productEmail=Database::search("SELECT * FROM `product` WHERE `id`='".$pid."'");
                    $productEmail_data=$productEmail->fetch_assoc();
                    $user3=Database::search("SELECT * FROM `user` WHERE `email`='".$productEmail_data["user_email"]."'");
                    $user3_data=$user3->fetch_assoc();
